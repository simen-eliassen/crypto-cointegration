...
class PriceDataDownloader:
    def __init__(
        self, database, exchange, timeframe, incremental_download=True, topN=100
    ):
        self.database = database
        self.exchange = exchange
        self.timeframe = timeframe
        self.incremental_download = incremental_download
        self.topN = topN

    def fetch_historical_data(
        self, row, exchange, timeframe="1d", from_date=None, limit=1000
    ):
        ...
        for _, row in tqdm.tqdm(df_symbols.iterrows(), total=df_symbols.shape[0]):
            ...
            while True:
                ...
                df = self.fetch_historical_data(
                    row, self.exchange, self.timeframe, from_date
                )
                dfs.append(df)
                # Check results
                if df.empty:
...
if __name__ == "__main__":
    ...
    downloader = PriceDataDownloader(
        database, exchange, timeframe, incremental_download, topN
    )
    downloader.download_price_data()
